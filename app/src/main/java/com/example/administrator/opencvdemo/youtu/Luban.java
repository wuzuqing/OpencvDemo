...
public class Luban {
    ...
    public  Luban cleanCache(){
        FileUtil.delete(mCacheDir.getAbsolutePath(),false);
        return INSTANCE;
    }
    public Luban launch() {
    ...
    public byte[] thirdCompress(File file,boolean saveFile) {
        String thumb = mCacheDir.getAbsolutePath() + File.separator +
                (TextUtils.isEmpty(filename) ? System.currentTimeMillis() + file.getName() : filename) ;
        ...
        Bitmap bitmap = compress(filePath, thumb, thumbW, thumbH, angle, (long) size, true);
     ...
     */
    private Bitmap compress(String largeImagePath, String thumbFilePath, int width, int height, int angle, long size,boolean saveFile) {
     ...
     * 压缩单张图片 Listener 方式
     */
    private void compressWithLs(File file, OnCompressListener onCompressListener) {
        Luban.get(BaseApplication.getAppContext())
                .load(file)
